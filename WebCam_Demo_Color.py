...
def get_font(font_size=36):
    # Function to get a font of the desired size
    try:
        # Try to load a TrueType font
        font = ImageFont.truetype("DejaVuSans-Bold.ttf", font_size)
    except IOError:
        try:
            # If not found, try to load from PIL's default fonts
            import PIL
            font_path = os.path.join(os.path.dirname(PIL.__file__), "fonts", "DejaVuSans-Bold.ttf")
            font = ImageFont.truetype(font_path, font_size)
        except IOError:
            # If all fails, use the default font
            font = ImageFont.load_default()
    return font


def plot_boxes_to_image(image_pil, tgt):
    H, W = tgt["size"]  # Image height and width
    boxes = tgt["boxes"]
    labels = tgt["labels"]
    ...
    # Load a larger font
    font = get_font(font_size=36)

    for box, label in zip(boxes, labels):
        # Convert box coordinates from normalized center to pixel coordinates
        box = box * torch.tensor([W, H, W, H])
        box[:2] -= box[2:] / 2  # Convert center coordinates to top-left
        ...
        text_x = x0
        text_y = y0 - text_size[1]

        # Adjust text position if it goes above the image
        if text_y < 0:
            text_y = y0

        # Draw text label with the same color as the bounding box
